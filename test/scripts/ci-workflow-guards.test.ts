...
describe("CI hand-port kept-ours contracts (card 30b64c3b)", () => {
  it("exposes the workflow_dispatch scope input [full, scoped, fallback]", () => {
    const source = readFileSync(".github/workflows/ci.yml", "utf8");
    expect(source).toContain("scope:");
    expect(source).toContain("description: CI scope for this dispatch.");
    expect(source).toContain("options: [full, scoped, fallback]");
  });

  it("wires dedicatedMaxLinesRatchet into the fast-core delegation", () => {
    const source = readFileSync(".github/workflows/ci.yml", "utf8");
    expect(source).toContain("dedicatedMaxLinesRatchet:");
    expect(source).toContain('task === "baseline-ratchets"');
  });

  it("preserves the max-lines ratchet budget at 496 lines from R1's slowest hosted check", () => {
    const source = readFileSync(".github/workflows/ci.yml", "utf8");
    expect(source).toContain("R1's slowest admitted hosted check took 496s");
    expect(source).toMatch(/check:max-lines-ratchet/);
  });

  it("retains changed-lane PR gating with workflow_dispatch + capacity fallback", () => {
    const source = readFileSync(".github/workflows/ci.yml", "utf8");
    expect(source).toContain("OPENCLAW_CI_RUNNER_BACKEND");
    expect(source).toContain("github.event_name == 'workflow_dispatch'");
    expect(source).toMatch(/changed-lane|CHANGED_NODE_TEST/);
  });
});
