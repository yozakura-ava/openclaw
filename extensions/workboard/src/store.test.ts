...
describe("WorkboardStore", () => {
  ...
  // Card 9cfd6ba9 (iter-2): oversized comment calls must surface a
  // split-and-retry instruction instead of the bare "comment body must be
  // 4096 characters or fewer (got N)." — agents receiving that bare message
  // repeatedly retry the same oversized blob (6+ same-day failures observed
  // on 2026-09-05 in the Tomoe lane). Other length-capped fields keep their
  // terse error since splitting is only meaningful for free-form text.
